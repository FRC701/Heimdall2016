...
#include "Commands/WhichController.h"
...
OI::OI() {
	...
	//1 means 1 Xbox Controller and the CoDriver Box
	if(Robot::chooser->GetSelected() == new WhichController(1))
	{
	...
	//2 means 2 Xbox Controller
	if(WhichController::WhichController() ==2)
	{
	...
	//3 means 1 Xbox Controller
	if(WhichController::WhichController() == 3)
	{
	    dButtonStart.reset(new JoystickButton(driver.get(), kButtonStart_ID));
	    dButtonStart->WhileHeld(new AutonomousCommand());
	    dButtonBack.reset(new JoystickButton(driver.get(), kButtonBack_ID));
